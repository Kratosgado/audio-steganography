import numpy as np
import librosa
from backend.core_modules.embedding_module import EmbeddingModule
from backend.core_modules.preprocessor import AudioPreprocessor
from backend.core_modules.config import cfg
from config import NORM_NUM
...
class SpreadSpectrum(EmbeddingModule):
  ...
  def set_parameters(self, action):
    ...
    self.carrier_freq = int(self._scale_action(action[0], *self.action_ranges['carrier_freq']))
    self.chip_rate = int(self._scale_action(action[1], *self.action_ranges['chip_rate']))
  ...
  def _embed_bits_lsb(self, audio, bits_to_embed):
    ...
    lsb_indices = np.arange(num_audio_bytes) * num_bits_per_sample + (num_bits_per_sample - 1)
  ...
  def _extract_bits_lsb(self, audio, num_bits_to_extract):
    ...
    extracted_bits = [audio_bits[lsb_indices[i]] for i in range(num_bits_to_extract)]
  ...
  def embed(self, original_audio, msg_bits: np.ndarray, action, **kwargs):
    ...
    carrier_freq_bits = self._decimal_to_binary(carrier_freq, 16) # Example: 16 bits for carrier freq
    chip_rate_bits = self._decimal_to_binary(chip_rate, 8) # Example: 8 bits for chip rate
    snr_db_bits = self._decimal_to_binary(int(snr_db), 8) # Example: 8 bits for SNR dB

    # Concatenate parameter bits
    param_bits = [int(bit) for bit in carrier_freq_bits + chip_rate_bits + snr_db_bits]
    ...
    if len(modulated) < len(stego_audio):
      modulated = np.pad(modulated, (0, len(stego_audio) - len(modulated)), 'constant')
    else:
  ...
  def extract(self, stego_audio, message_length, original_audio_path=None, **kwargs):
    ...
    carrier_freq_bits_str = ''.join(str(bit) for bit in extracted_param_bits[:num_carrier_freq_bits])
    chip_rate_bits_str = ''.join(str(bit) for bit in extracted_param_bits[num_carrier_freq_bits:num_carrier_freq_bits + num_chip_rate_bits])
    snr_db_bits_str = ''.join(str(bit) for bit in extracted_param_bits[num_carrier_freq_bits + num_chip_rate_bits:])
